#!/bin/bash
options=$(getopt -o h:u:p:v:i:r:c:proxy:port --long host:,user:,password:,version:,image:,repository:,:cluster,proxy:port -- "$@")
eval set -- "$options"
...
for ((i=0; i<cluster; i++))
do
    mkdir -p $PWD/nacos-$i/logs && mkdir -p $PWD/nacos-$i/data;
    nacosServers
done



curl ${proxy}${repository}/${version}/nacos/nacos-cluster.yaml -o $PWD/docker-compose.yml;
